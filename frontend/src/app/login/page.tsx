...
export default function Login() {
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center relative overflow-hidden bg-slate-50">
      ...
      <div className="absolute top-[-10%] left-[10%] w-[500px] h-[500px] bg-red-100/50 rounded-full mix-blend-multiply filter blur-[100px] opacity-70 pointer-events-none" />
      <div className="absolute bottom-[-10%] right-[10%] w-[500px] h-[500px] bg-gray-200/50 rounded-full mix-blend-multiply filter blur-[100px] opacity-70 pointer-events-none" />

      {/* Botão de voltar para a Home */}
      <div className="absolute top-8 left-8 z-20">
        <Link
          href="/"
          className="flex items-center gap-2 text-sm font-medium text-gray-500 hover:text-red-500 transition-colors bg-white/50 px-4 py-2 rounded-full backdrop-blur-sm shadow-sm"
        >
      ...
      <div className="relative z-10 w-full max-w-md mx-4 p-8 sm:p-10 bg-white/60 backdrop-blur-2xl border border-gray-400/80 shadow-[0_20px_40px_-15px_rgba(0,0,0,0.05)] rounded-[2.5rem]">
        
        {/* Logo da JIMUE */}
        <div className="flex justify-center mb-6">
          <div className="w-16 h-16 bg-white shadow-sm border border-gray-100 rounded-2xl flex items-center justify-center overflow-hidden p-2.5">
            <img
              src="/JIMUE-logo.jpg"
              alt="Logo JIMUE"
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-4 bg-white/60 border border-gray-200/80 rounded-2xl focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent transition-all text-gray-800 placeholder-gray-400 shadow-sm"
            />
          </div>
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full bg-gray-900 cursor-pointer hover:bg-black text-white font-semibold py-4 px-4 rounded-2xl transition-all duration-300 flex items-center justify-center disabled:opacity-70 disabled:cursor-not-allowed shadow-lg shadow-gray-900/20 mt-2"
          >
            {isLoading ? (
              <>
      ...
      {activeModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl border border-gray-100 p-6 sm:p-8 relative text-center animate-in zoom-in-95 duration-200">
            <button
            ...
            <button
              onClick={() => setActiveModal(null)}
              className="w-full bg-gray-900 hover:bg-black text-white font-semibold py-3.5 px-4 rounded-xl transition-all duration-200 shadow-md cursor-pointer"
            >
              Entendido
            </button>
          </div>
